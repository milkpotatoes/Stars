...
const MATERIAL_ICON_STYLE = ".material-icon{font-family:material-icon}.icon{width:20px;height:20px;user-select:none}span.icon{width:32px;height:32px;font-size:20px;line-height:32px;border-radius:4px;text-align:center;transition:ease-in-out .2s}"
...
function discordChanges() {
    return new Promise((resolve, reject) => {
        new AlertDialog()
            .setTitle($i18n('dialog-title-notice'))
            .setMessage($i18n('{{discord-changes-and-exit}}'))
...
export class SearchEngineManager {
    ...
    static checkEngine(engine, alert = true) {
        ...
        if (icon === '' || icon === default_icon_src) {
            if (alert) alertMessage($i18n('{{icon-is-required}}'));
            return false;
        };
        if (name === '' || url === '' || index === '') {
            if (alert) alertMessage($i18n('{{missing-required-fields}}'));
            return false;
        };
        if (!url.match(/%s/)) {
            if (alert) alertMessage($i18n('{{search-keyword-is-required}}'));
            return false;
        };
        return true;
    }
    ...
    editSearchIcon(icon = '', logo = '', url = '', editable = true) {
        return new Promise((resolve, reject) => {
            const customView = `<style>${MATERIAL_ICON_STYLE}
.container{display:grid;grid-template-columns:1fr 32px 96px 1fr;grid-template-rows:128px 32px;gap:16px;grid-template-areas:"l l l l" ". i b .";align-items:center;justify-items:center}
div.icon{grid-area:i;width:32px;height:32px;user-select:none;background-image:url(../src/image_FILL0_wght400_GRAD0_opsz24.svg);border-radius:50%;overflow:hidden;padding:4px}
.logo{grid-area:l;color:rgba(0 0 0 / .4);font-size:2em;width:100%;height:100%;display:flex;align-items:center;justify-content:center;padding:16px}
.logo,div.icon{box-sizing:border-box;background:rgba(0 0 0 / .15) center no-repeat;background-size:contain;background-origin:content-box}
button.use-favicon{grid-area:b}</style>
<div class="container"> <input type="file" accept="image/*" style="display: none;" /> <div class="logo">{{click-to-set-logo}}</div> <div class="icon"></div><button class="use-favicon">{{use-favicon}}</button> </div>`;
            new AlertDialog()
                .setTitle((editable ? $i18n('{{dialog-title-edit}}')
                    : $i18n('{{dialog-title-view}}')))
    ...
    genISEConfig({ name, url, index, disabled }, id) {
        const uniTemplate = '<dd class="${type} ${status}" data="${id}"><img class="icon" src="${icon}">' +
            '<input type="text" placeholder="{{text-name}}" class="name" value="${name}" ${disabled}>' +
            '<input type="text" placeholder="{{search-engine-url}}" class="url" value="${url}" ${disabled}>' +
            '<input type="text" placeholder="{{search-engine-homepage}}" class="index" value="${index}" ${disabled}>' +
            '<span class="icon material-icon ${action}"></span>' +
            '</dd>';
        return uniTemplate
    ...
    genCSEConfig({ name, url, index }, id) {
        const uniTemplate = '<dd class="${type} ${status}" data="${id}">' +
    ...
    manageSearchEngines() {
        ...
        const saveChanges = function () {
            ...
            if (illegal.length > 0) {
                let sortedEngines = illegal.map(e => e.name)
                alertMessage($i18n('{{invalid-search-engine-configures}}', sortedEngines.length, sortedEngines.join(', ')));
                return true;
            } else {
                this_.startProfile.saveCustomSearchEngine();
                return buttonListener.apply(this);
...
dd{margin:0;display:grid;grid-template-columns:24px .4fr 1fr .8fr 24px;gap:8px;align-items:center}
dt{opacity:.8;font-size:.8em;padding-top:8px}
span.icon:hover{background:rgba(0 0 0 / .2)}
.icon.block::before{content:"\\e14b"}
.icon.remove::before{content:"\\e14c"}
.icon.add::before,.icon.restore::before{content:"\\e145"}
.hide-disabled .disabled{display:none}
.disabled{order:1}
.disabled>*{opacity:.6}
.disabled>.icon.restore{opacity:1}
dt{user-select:none;pointer:default}
.hide-disabled dt::before{content:"►"}
.show-disabled dt::before{content:"▼"}</style>
<div class="hide-disabled">
<dl>
${searchEngine}
<dd class="add-item">
<img class="icon" src="${default_icon}">
<input type="text" placeholder="{{text-name}}" class="name">
<input type="text" placeholder="{{search-engine-url}}" class="url" value="">
<input type="text" placeholder="{{search-engine-homepage}}" class="index" value="">
<span class="icon material-icon add"></span>
</dd>
<dt>{{disabled-inner-search-engines}}</dt>
</dl>
</div>`;
        const this_ = this;
        new AlertDialog()
            .setTitle($i18n('{{index-manage-search-engines}}'))
            .setView($i18n(customView))
            .setPositiveButton($i18n('{{button-text-confirm}}'), saveChanges)
            .setNegativeButton($i18n('{{button-text-cancel}}'), cancelChanges)
            .setModal(true)
            .onShow((view) => {
